...
// Import dependencies (also available on window for backward compatibility)
import { saltmereTracker } from './error-tracker.js';
import { WordHighlight } from './word-highlight.js';
  ...
  if (readAlongBtn) {
    ...
    readAlongBtn.addEventListener('click', () => {
      ...
      if (wordHighlightAvailable) {
        ...
        if (WordHighlight.isFollowActive()) {
          WordHighlight.disableFollow();
        } else {
          WordHighlight.enableFollow();
      ...
      // Track usage
      if (saltmereTracker) {
        saltmereTracker.trackAudio('follow-toggle', {
...
async function initWordHighlight(chapterData, callbacks = {}) {
  ...
  return await WordHighlight.init(chapterNum, audio, options);
}
...
export { init, getChapterKey, formatTime };
